...
class AlignLBModule(LightningModule):
    def __init__(self, model, enc_tokenizer, lm_tokenizer, multi_lmtokenizer, args):
        super().__init__()
        self.model: LangBridgeModel = model
        self.enc_tokenizer = enc_tokenizer
        self.lm_tokenizer = lm_tokenizer
        self.multi_lmtokenizer = multi_lmtokenizer
        self.args = args
    ...
    def collate_fn(self, batch, use_dynamic=False):
        ...
        multi_lmtokens = self.multi_lmtokenizer(
            suffix, max_length=max_length, truncation=True, padding='max_length', return_tensors='pt')
        labels_multi = multi_lmtokens['input_ids'].clone().detach()
        labels_multi[labels_multi == self.multi_lmtokenizer.pad_token_id] = -100
    ...
    def collate_fn_output_exists(self, batch):
        ...
        return {
            ...
            'input_ids': lm_tokens['input_ids'],
            'attention_mask': lm_tokens['attention_mask'],
            'multi_input_ids': multi_lmtokens['input_ids'],
            'multi_attention_mask': multi_lmtokens['attention_mask'],
            'labels': labels,
            'multi_labels': labels_multi,
        }
...
if __name__ == '__main__':
    ...
    try:
        multi_lmtokenizer = AutoTokenizer.from_pretrained(
            training_args.multi_lm, use_fast=False)
    except:
        multi_lmtokenizer = AutoTokenizer.from_pretrained(
            training_args.multi_lm)

    lm_tokenizer.padding_side = 'right'
    multi_lmtokenizer.padding_side = 'right'

    if not enc_tokenizer.pad_token:
        enc_tokenizer.pad_token = enc_tokenizer.eos_token
    if not lm_tokenizer.pad_token:
        lm_tokenizer.pad_token = lm_tokenizer.eos_token
    if not multi_lmtokenizer.pad_token:
        multi_lmtokenizer.pad_token = multi_lmtokenizer.eos_token
    ...
    config = LangBridgeConfig(
        ...
        multi_lm=training_args.multi_lm,
        alignments=training_args.alignments,
        alignments1=training_args.alignments1,
        dim_enc=training_args.enc_hidden_size,
        dim_lm=training_args.lm_hidden_size,
        dim_multi_lm=training_args.dim_multi_lm,
    ...
    pl_model = AlignLBModule(model, enc_tokenizer,
                             lm_tokenizer,multi_lmtokenizer, training_args)
